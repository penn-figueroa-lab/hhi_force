...
class OPF_3d:
    ...
    '''
    def update_all(self, measurements):
        """
        measurements: list of np.array of shape (6,)
        """
        self.weights.fill(1.)
        self.weights1.fill(1.)

        for m in measurements:
            distance = np.linalg.norm(self.particles - m[:3], axis=1)
            distance1 = np.linalg.norm(self.particles1 - m[3:], axis=1)

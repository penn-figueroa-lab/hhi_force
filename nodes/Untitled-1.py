...
# Use all umi from cam1 + base from cam1 or fallback
    for T_tag in base_poses_1:
        trans_b = T_tag[:3,3]
        quat_wxyz = t3d.quaternions.mat2quat(T_tag[:3,:3])
        quat_b = np.roll(quat_wxyz, -1)
        base_measurements.append({
            'value': np.hstack((trans_b, quat_b)),
            'weight': 1.0
        })
    # from camera 2
    for T_tag in base_poses_2:
        trans_b = T_tag[:3, 3]
        quat_wxyz = t3d.quaternions.mat2quat(T_tag[:3, :3])
        quat_b    = np.roll(quat_wxyz, -1)
        base_measurements.append({
  ...
  # ─── 2) Update or Predict the base filter ─────────────────────────────────
    if base_measurements:
        # we have at least one new tag → fuse & smooth
        ...
        T_base = np.eye(4)
        T_base[:3, 3]   = obj_OPF_base.curr_pos
        # OPF gives [x,y,z,w], but mat2quat needs [w,x,y,z]
        q = obj_OPF_base.curr_pos1
        quat_wxyz = np.array([q[3], q[0], q[1], q[2]])
        T_base[:3, :3] = t3d.quaternions.quat2mat(quat_wxyz)
        # update both fallbacks
        last_known_base_pose_1 = T_base.copy()
        last_known_base_pose_2 = T_base.copy()
    else:
        # no new base tag → only predict (keeps OPF’s internal state evolving)
        obj_OPF_base.predict()
        # last_known_base_pose_* remain as the last stable pose
        camera1_to_base = np.linalg.inv(last_known_base_pose_1)
        camera2_to_base = np.linalg.inv(last_known_base_pose_2)
    ...
    for det in detections1:
        ...
        if len(umi_poses_1) > 0 and f'from_{tag_id}' in tag_to_gripper:
            ...
            for umi_pose in umi_poses_1:
                ...
                quat = t3d.quaternions.mat2quat(rot_matrix)  # [w, x, y, z]
                quat = np.roll(quat, -1)  # → [x, y, z, w]
                measurement = {
                    'value': np.array([*trans, *quat]),
                    'weight': 1.0  # Placeholder confidence
                }
    ...
    filtered_quat = obj_OPF.curr_pos1
    if filtered_quat.shape[0] != 4:
        rospy.logwarn_throttle(1.0, "Filtered quaternion is invalid. Falling back to identity.")
        filtered_quat = np.array([0, 0, 0, 1])  # [x, y, z, w] → identity quaternion

    filtered_rot_matrix = t3d.quaternions.quat2mat(np.roll(filtered_quat, 1))  # [w, x, y, z]

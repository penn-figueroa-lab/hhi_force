...
class OPF_3d:
    def __init__(self, num_particles=5000, name=None, objid=None):
        ...
        rand_quats = np.random.randn(self.num_particles, 4)
        rand_quats /= np.linalg.norm(rand_quats, axis=1, keepdims=True)
        self.particles1 = rand_quats
        ...
        self.dyn_thresh = 0.01
        self.hidden_factor = 1
        self.static = 0
        self.cov_list = []

        # Trajectory and prediction
        self.trajectory = [np.array([0, 0, 0, 0, 0, 0, 1]), np.array([0, 0, 0, 0, 0, 0, 1])]
        self.gt_pos = [np.array([0, 0, 0, 0, 0, 0, 1]), np.array([0, 0, 0, 0, 0, 0, 1])]
        self.future_pred = []
        self.hidden_count = 0


    def predict(self):
        std = np.array([0.01, 0.1])  # translational noise
        vec = np.array(self.trajectory[-1][:3] - self.trajectory[-2][:3]) / 5

        for i in range(3):
            self.particles[:, i] += vec[i] + np.random.randn(self.num_particles) * std[1]

        # Quaternion random walk via axis-angle jitter
        for i in range(self.num_particles):
            axis = np.random.randn(3)
            axis /= np.linalg.norm(axis)
            angle = np.random.normal(0, 0.05)  # 0.05 rad noise
            delta_q = R.from_rotvec(axis * angle).as_quat()  # [x, y, z, w]
            q = R.from_quat(self.particles1[i])
            self.particles1[i] = (q * R.from_quat(delta_q)).as_quat()
    ...
    '''
    def update_all(self, measurements, sigma_pos=None, sigma_ori=None, angle_thresh=np.pi / 2):
        """
        measurements: list of dicts {'value': np.array([x, y, z, qx, qy, qz, qw]), 'weight': float}
        """

        sigma_pos = sigma_pos or self.R
        sigma_ori = sigma_ori or self.R1
        ...
        for m in measurements:
            ...
            v = m['value']
            pos = v[:3]
            quat_meas = v[3:7] / np.linalg.norm(v[3:7])

            # --- Position update ---
            dist = np.linalg.norm(self.particles - pos, axis=1)
            self.weights *= np.power(norm(loc=0, scale=sigma_pos).pdf(dist), w)

            # --- Orientation update ---
                        # Align signs of all particles with the measurement
            if quat_meas.shape[0] != 4:
                rospy.logwarn("Invalid quat_meas shape. Expected 4, got {}".format(quat_meas.shape))
                continue
            if self.particles1.shape[1] != 4:
                raise ValueError("particles1 must be quaternion shape (N,4), found {}".format(self.particles1.shape))

            aligned_particles = np.array([
                q if np.dot(q, quat_meas) > 0 else -q
                for q in self.particles1
            ])

            dot = np.sum(aligned_particles * quat_meas, axis=1)
            dot = np.clip(dot, -1.0, 1.0)
            angle = 2 * np.arccos(dot)

            penalty = np.where(angle > angle_thresh, 1e-4, 1.0)
            self.weights1 *= np.exp(-0.5 * (angle / sigma_ori) ** 2 * w) * penalty

        # Normalize
        self.weights += 1e-300
        self.weights /= np.sum(self.weights)

        self.weights1 += 1e-300
        ...
        aligned_particles = np.array([
            q if np.dot(q, self.particles1[0]) > 0 else -q
            for q in self.particles1
        ])
        weighted_quats = aligned_particles * self.weights1[:, np.newaxis]
        mean_quat = np.sum(weighted_quats, axis=0)
        mean_quat /= np.linalg.norm(mean_quat)
        self.curr_pos1 = mean_quat
    ...
    def update_orientation_quaternion(self, measurement_quat, sigma=0.1):
        """
        Update the orientation particles using a quaternion-based angular distance metric.
        :param measurement_quat: np.array([x, y, z, w])
        :param sigma: noise std deviation (radians)
        """
        # Normalize input quaternion
        measurement_quat = measurement_quat / np.linalg.norm(measurement_quat)

        # Compute angular distance
        dots = np.abs(np.sum(self.particles1 * measurement_quat, axis=1))
        dots = np.clip(dots, -1.0, 1.0)
        angles = 2 * np.arccos(dots)  # in radians

        # Likelihood weighting
        self.weights1 = np.exp(-0.5 * (angles / sigma) ** 2)
        self.weights1 += 1e-300  # to avoid division by zero
        self.weights1 /= np.sum(self.weights1)

        # Compute weighted average quaternion
        mean_quat = np.average(self.particles1, axis=0, weights=self.weights1)
        mean_quat /= np.linalg.norm(mean_quat)
        ...
        if np.linalg.norm(self.trajectory[-10][:3] - self.trajectory[-1][:3]) > self.dyn_thresh and not self.static:
            ...
            if self.hidden_factor == 1:
                ...
                prev_axis = axangle[0][0]
                                ...
                                # Get the last 50 quaternions
                quaternions = np.array(self.trajectory[-50:])[:, 3:7]  # shape: (50, 4)

                # Normalize all to ensure valid quaternions
                quaternions = quaternions / np.linalg.norm(quaternions, axis=1, keepdims=True)

                # Align signs to avoid flipping (double-cover)
                for i in range(1, len(quaternions)):
                    if np.dot(quaternions[i], quaternions[i-1]) < 0:
                        quaternions[i] = -quaternions[i]

                # Use last two as base and target for extrapolation
                q0 = quaternions[-2]
                q1 = quaternions[-1]
                r0 = R.from_quat(q0)
                r1 = R.from_quat(q1)

                # Predict 950 time steps into the future using SLERP
                slerp = R.slerp(0, 1, [r0, r1])
                num_steps = 950
                times = np.linspace(1.05, 2.0, num_steps)  # extrapolate slightly beyond last known

                future_rots = slerp(times)
                future_quats = future_rots.as_quat()

                # Predict future positions (same as before)
                for i, t in enumerate(range(51, 1001)):
                    px = x_coef[0] + x_coef[1] * t
                    py = y_coef[0] + y_coef[1] * t
                    pz = z_coef[0] + z_coef[1] * t
                    quat = future_quats[i]
                    self.future_pred.append(np.hstack((px, py, pz, quat)))
...
class PF_3d:
    def __init__(self, num_particles=1000, name=None, objid=None):
        ...
        # Translation particles (x, y, z)
        self.particles = np.random.uniform(-1, 1, size=(self.num_particles, 3))

        # Quaternion particles (x, y, z, w)
        rand_quats = np.random.randn(self.num_particles, 4)
        rand_quats /= np.linalg.norm(rand_quats, axis=1, keepdims=True)
        self.particles1 = rand_quats

        # Trajectory log: [x y z qx qy qz qw]
        self.trajectory = [np.array([0, 0, 0, 0, 0, 0, 1])]
        ...
        # Noise scales
        self.R = 0.1      # position noise
        self.R1 = 0.1     # orientation noise

        # Current pose estimate
        self.curr_pos = np.average(self.particles, weights=self.weights, axis=0)

        weighted_quats = self.particles1 * self.weights1[:, np.newaxis]
        avg_quat = np.sum(weighted_quats, axis=0)
        avg_quat /= np.linalg.norm(avg_quat)
        self.curr_pos1 = avg_quat

        # Covariance for uncertainty ellipse (XY plane)
        self.cov = np.cov(self.particles[:, :2].T, aweights=self.weights)
